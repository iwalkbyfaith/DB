...
(WHERE구문은 ON으로 합쳐진 결과 컬럼에 대한 필터링을 해줍니다. = WHERE 대신 ON)

INNER JOIN		 = 양쪽 테이블 모두에 존재하는 데이터만 출력
LEFT JOIN 		 = 왼쪽 테이블 데이터는 매칭이 되건 말건 다 살림, 오른쪽 테이블 데이터는 왼쪽에 존재하는 것만 출력
RIGHT JOIN 		 = 오른쪽 테이블 데이터는 무조건 다 살림, 왼쪽 테이블은 오른쪽에 존재하는 것만.
